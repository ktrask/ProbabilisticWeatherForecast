...
#Only reached if a caller bypasses the view, which requires a location.
DEFAULT_LATITUDE = 52.2646577
DEFAULT_LONGITUDE = 10.5236066


def plotMeteogramFile(latitude = None, longitude = None, altitude = None, location = None, days = 3, plotType = "enhanced-hres"):
    ...
    #Open-Meteo reports the elevation of the grid cell it used, so there is no
    #need for a second round trip to an elevation service that may be down.
    metadata = {}
    if days <= 10:
        allMeteogramData = getData(float(longitude), float(latitude), altitude, writeToFile = False, metadata = metadata)
    else:
        allMeteogramData = getData(float(longitude), float(latitude), altitude, writeToFile = False, meteogram = "15days", metadata = metadata)
    altitude = metadata.get("elevation")
    if altitude is None:
    ...
    fig.suptitle(location + " " + str(np.round(latitude, decimals = 2)) +\
                  ...
                  "°/" + str(int(round(altitude))) + "m", fontproperties=prop)
